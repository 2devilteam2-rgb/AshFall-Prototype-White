...
public sealed partial class RestorationSystem : SharedRestorationSystem
{
    ...
    private readonly Color _ghostColor = new(255, 128, 0, 128);

    private readonly EntProtoId _restorationGhost = "AshfallRestorationGhost";

    private ITileDefinition? _platingDefinition;
    ...
    public override void Update(float frameTime)
    {
        ...
        if (player == null || !HasComp<RestorationEyeComponent>(player))
        {
            ClearGhosts();
            return;
        }
        ...
        float maxRange = 0f;
        foreach (var item in _hands.EnumerateHeld(player.Value))
            if (_toolQuery.TryComp(item, out var tool) && tool.GhostRenderRadius > maxRange)
        ...
        foreach (var grid in grids)
        {
            ...
            var minChunk = GetChunkIndex(minTile, data.ChunkSize);
            var maxChunk = GetChunkIndex(maxTile, data.ChunkSize);
            ...
            for (var x = minChunk.X; x <= maxChunk.X; x++)
            {
                for (var y = minChunk.Y; y <= maxChunk.Y; y++)
                {
                    ...
                    {
                        if (spec.OriginalEntity is { } original && TryGetEntity(original, out var origUid) &&
        ...
        foreach (var key in _visibleGhosts)
        {
            ...
            if (grid.Comp1.Chunks.TryGetValue(chunkIdx, out var chunk))
            {
                ...
                {
                    var protoId = grid.Comp1.EntityPalette[spec.PrototypeIndex];
    ...
    private void SpawnEntityGhost(GhostPosData key, RestorationEntitySnapshot spec, EntProtoId protoId)
    {
            ...
            && proto.TryGetComponent<SpriteComponent>(out var specSprite, Factory))
        {
            ...
            var ghost = Spawn(_restorationGhost, new EntityCoordinates(key.Grid, Vector2.Zero));
            _transform.SetCoordinates(ghost, new EntityCoordinates(key.Grid, spec.LocalPosition));
            ...
            _sprite.SetColor(ent, _ghostColor);
            ...
            _metaData.SetEntityName(ghost, Loc.GetString("ashfall-restoration-ghost-name", ("proto", proto.Name)));
    ...
    {
        _platingDefinition ??= _tileDefinitions["Plating"];

        if (_tileDefinitions.TryGetDefinition(tileId, out var def)
            && def is ContentTileDefinition tileDef)
        {
            var localPos = _map.TileCenterToVector((key.Grid, key.Grid.Comp2), indices);
            var ghost = Spawn(_restorationGhost, new EntityCoordinates(key.Grid, Vector2.Zero));
            _transform.SetCoordinates(ghost, new EntityCoordinates(key.Grid, localPos));
            ...
            if (_platingDefinition?.Sprite != null)
            {
                var layer = _sprite.AddBlankLayer(ent, 0);
                _sprite.LayerSetTexture(ent, 0, _platingDefinition.Sprite.Value);
                _sprite.LayerSetVisible(layer, true);
                sprite.LayerSetShader(0, "unshaded");
            }

            _sprite.SetColor(ghost, _ghostColor);
            _sprite.SetDrawDepth(ghost, (int)Content.Shared.DrawDepth.DrawDepth.FloorObjects);

            _metaData.SetEntityName(ghost, Loc.GetString("ashfall-restoration-ghost-name", ("proto", Loc.GetString(tileDef.Name))));

            _activeGhosts[key] = ghost;
        }
    }

    private record struct GhostPosData(Entity<RestorationGridComponent, MapGridComponent> Grid, Vector2i ChunkIndices, int Id, bool IsTile);
}
