...
public sealed class UniversalRecorderSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<UniversalRecorderComponent, ComponentShutdown>(OnRecorderShutdown);
    ...
    private void OnRecorderShutdown(Entity<UniversalRecorderComponent> ent, ref ComponentShutdown args)
    {
        // Stop() early-returns once Stopped, so this stays safe alongside OnRecorderTerminating.
    ...
    private void OnRecorderExamined(Entity<UniversalRecorderComponent> ent, ref ExaminedEvent args)
    {
        ...
        {
            args.PushMarkup(Loc.GetString("ashfall-recorder-examine-actions"));

            if (!TryGetTape(ent, out var tape))
            {
                args.PushMarkup(Loc.GetString("ashfall-recorder-examine-display", ("value", Loc.GetString("ashfall-recorder-readout-empty"))));
                return;
            }

            var tapeRuntime = GetTapeRuntime(tape);
            args.PushMarkup(Loc.GetString("ashfall-recorder-examine-tape", ("tape", tape.Owner)));
            args.PushMarkup(Loc.GetString("ashfall-recorder-examine-display", ("value", GetReadout(ent, tapeRuntime))));

            if (tapeRuntime.Unspooled)
                args.PushMarkup(Loc.GetString("ashfall-recorder-examine-broken"));
        }
    }
    ...
    private void OnTapeExamined(Entity<UniversalRecorderTapeComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(UniversalRecorderTapeComponent)))
        {
            ...
            args.PushMarkup(Loc.GetString("ashfall-recorder-tape-side", ("side", GetSideName(tapeRuntime.Side))));

            if (tapeRuntime.Unspooled)
            {
                args.PushMarkup(Loc.GetString("ashfall-recorder-tape-unspooled"));
                return;
            }
            ...
            {
                <= 5 => "ashfall-recorder-tape-unused",
                <= 25 => "ashfall-recorder-tape-bit-used",
                <= 50 => "ashfall-recorder-tape-under-half",
                <= 75 => "ashfall-recorder-tape-over-half",
                <= 90 => "ashfall-recorder-tape-almost-full",
                <= 99 => "ashfall-recorder-tape-tiny-bit-left",
                _ => "ashfall-recorder-tape-fully-used",
            };
    ...
    private void OnRecorderUseInHand(Entity<UniversalRecorderComponent> ent, ref UseInHandEvent args)
    {
        ...
        if (!TryGetTape(ent, out var tape))
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-no-tape"), ent.Owner, args.User);
            args.Handled = true;
            return;
        }
        ...
        if (tapeRuntime.Unspooled)
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-broken"), ent.Owner, args.User);
            args.Handled = true;
            return;
        }
    ...
    private void OnRecorderInteractUsing(Entity<UniversalRecorderComponent> ent, ref InteractUsingEvent args)
    {
        ...
        _popup.PopupEntity(
            Loc.GetString("ashfall-recorder-popup-insert", ("tape", args.Used)),
            ent.Owner,
            args.User);
        args.Handled = true;
    }
    ...
    private void OnRecorderGetVerbs(Entity<UniversalRecorderComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        {
            Text = Loc.GetString("ashfall-recorder-verb-controls"),
            Priority = 3,
        ...
        if (runtime.State == UniversalRecorderState.Playing || runtime.State == UniversalRecorderState.Recording)
        {
            args.Verbs.Add(new AlternativeVerb
            {
                Text = Loc.GetString("ashfall-recorder-verb-stop"),
                Priority = 2,
                Act = () => Stop(ent),
            });
        }
        else if (!tapeRuntime.Unspooled)
        {
            if (tapeRuntime.Entries.Count > 0)
            {
                args.Verbs.Add(new AlternativeVerb
                {
                    Text = Loc.GetString("ashfall-recorder-verb-play"),
                    Priority = 2,
                ...
                args.Verbs.Add(new AlternativeVerb
                {
                    Text = Loc.GetString("ashfall-recorder-verb-print"),
            ...
            args.Verbs.Add(new AlternativeVerb
            {
                Text = Loc.GetString("ashfall-recorder-verb-record"),
    ...
    private void OnRecorderBuiAction(Entity<UniversalRecorderComponent> ent, ref UniversalRecorderRecorderActionBuiMsg args)
    {
        ...
        switch (args.Action)
        {
            ...
            case UniversalRecorderRecorderAction.Eject:
                ...
                {
                    _popup.PopupEntity(
                        Loc.GetString("ashfall-recorder-popup-eject", ("tape", tape.Owner)),
                        ent.Owner,
                        args.Actor);
                }
                break;
    ...
    private bool TryOpenRecorderUi(Entity<UniversalRecorderComponent> ent, EntityUid user)
    {
        ...
        if (!TryGetTape(ent, out var tape))
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-no-tape"), ent.Owner, user);
            return false;
        }

        if (GetTapeRuntime(tape).Unspooled)
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-broken"), ent.Owner, user);
            return false;
        }
    ...
    private void OnTapeGetVerbs(Entity<UniversalRecorderTapeComponent> ent, ref GetVerbsEvent<AlternativeVerb> args)
    {
        ...
        args.Verbs.Add(new AlternativeVerb
        {
            Text = Loc.GetString("ashfall-recorder-tape-verb-flip"),
            Priority = 2,
        ...
        if (!GetTapeRuntime(ent).Unspooled)
        {
            args.Verbs.Add(new AlternativeVerb
            {
                Text = Loc.GetString("ashfall-recorder-tape-verb-unwind"),
                Priority = 1,
    ...
    private void OnTapeInteractUsing(Entity<UniversalRecorderTapeComponent> ent, ref InteractUsingEvent args)
    {
        ...
        _popup.PopupEntity(Loc.GetString("ashfall-recorder-tape-popup-respool-start"), ent.Owner, args.User);
    ...
    private void OnTapeRespoolDoAfter(Entity<UniversalRecorderTapeComponent> ent, ref UniversalRecorderTapeRespoolDoAfterEvent args)
    {
        if (args.Cancelled)
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-tape-popup-respool-cancel"), ent.Owner, args.User);
            return;
        }

        RespoolTape(ent);
        _popup.PopupEntity(Loc.GetString("ashfall-recorder-tape-popup-respool-finish"), ent.Owner, args.User);
    }
    ...
    private void OnRecorderListen(Entity<UniversalRecorderComponent> ent, ref ListenEvent args)
    {
        ...
        if (currentDuration >= tape.Comp.MaxCapacity)
        {
            ...
            SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-full"));
            Stop(ent);
            return;
        }
    ...
    private bool StartRecording(Entity<UniversalRecorderComponent> ent, EntityUid user)
    {
        ...
        SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-recording-start"));
        UpdateAppearance(ent);
        return true;
    }
    ...
    private bool StartPlayback(Entity<UniversalRecorderComponent> ent, EntityUid user)
    {
        ...
        if (tapeRuntime.Entries.Count == 0)
        {
            SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-no-data"));
            return false;
        }
        ...
        EnsureComp<ActiveUniversalRecorderComponent>(ent);
        _audio.PlayPvs(ent.Comp.PlaySound, ent);
        SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-playback-start"));
        UpdateAppearance(ent);
        return true;
    }
    ...
    private void Stop(Entity<UniversalRecorderComponent> ent, bool suppressStatus = false)
    {
        ...
        if (!suppressStatus)
        {
            var message = previous == UniversalRecorderState.Recording
                ? "ashfall-recorder-popup-recording-stop"
                : "ashfall-recorder-popup-playback-stop";
    ...
    private void UpdateRecording(Entity<UniversalRecorderComponent, UniversalRecorderRuntimeComponent> ent)
    {
        ...
        if (!runtime.WarningSent && remaining <= ent.Comp1.WarningThreshold)
        {
            ...
            SendRecorderNotice(ent,
                Loc.GetString("ashfall-recorder-popup-warning",
    ...
    private void UpdatePlayback(Entity<UniversalRecorderComponent, UniversalRecorderRuntimeComponent> ent)
    {
        ...
        {
            runtime.PendingSilenceSeconds = null;
            SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-playback-silence", ("seconds", silenceSeconds)));
        ...
        {
            SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-end"));
            Stop(ent, suppressStatus: true);
            return;
        }
    ...
    private bool PrintTranscript(Entity<UniversalRecorderComponent> ent, EntityUid user)
    {
        ...
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-print-cooldown"), ent.Owner, user);
            return false;
        }
        ...
        if (tapeRuntime.Entries.Count == 0)
        {
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-popup-no-data"), ent.Owner, user);
            return false;
        }
        ...
        AppendLineLimited(text, Loc.GetString("ashfall-recorder-transcript-header"), paperComp.ContentSize);
        ...
        _metaData.SetEntityName(printed, Loc.GetString("ashfall-recorder-transcript-name", ("tape", tape.Owner)));
        ...
        SendRecorderNotice(ent, Loc.GetString("ashfall-recorder-popup-print"));
        return true;
    }
    ...
    private void FlipTape(Entity<UniversalRecorderTapeComponent> ent, EntityUid? popupUser = null, bool playSound = true)
    {
        ...
        if (popupUser != null)
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-tape-popup-flip"), ent.Owner, popupUser.Value);
    ...
    private void UnspoolTape(Entity<UniversalRecorderTapeComponent> ent, EntityUid? user = null, bool popup = false)
    {
        ...
        if (popup && user != null)
            _popup.PopupEntity(Loc.GetString("ashfall-recorder-tape-popup-unwind"), ent.Owner, user.Value);
    }
        ...
        UniversalRecorderTapeRuntimeComponent tapeRuntime)
    {
        if (GetRecorderRuntime(ent).State == UniversalRecorderState.Playing)
            return Loc.GetString("ashfall-recorder-readout-playing");

        return Loc.GetString("ashfall-recorder-readout-time",
    ...
    private string GetSideName(UniversalRecorderTapeSide side)
    {
        var key = side == UniversalRecorderTapeSide.Front
            ? "ashfall-recorder-tape-side-front"
            : "ashfall-recorder-tape-side-back";
    ...
    {
        return Exists(user) && _actionBlocker.CanInteract(user, null);
    }
